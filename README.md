...
- **Discrepancy stamp** — compares cash+M-Pesa actually collected
  ...
  indicator. A credit sale no longer looks like a shortfall.
- **Debts** — tracks customer credit ("on the tab"): who owes what, when,
  payments received over time, and outstanding balance per customer.
...
money) being reconciled — the normal case for a small pub. If an expense is
ever paid from a separate account entirely, it will show up as a small
"Over" on that day, which is expected and fine to ignore.
...
3. Open **SQL Editor** → **New query**, paste the entire contents of
   ...
   the `items`, `day_cash`, `day_shifts`, `stock_entries`, `day_expenses`,
   `debts`, and `debt_payments` tables.
   > Already have an older version of this project running? `schema.sql` only
...
| GET    | /api/days/:date      | get one day's full record (incl. expenses) |
...
| GET    | /api/debts           | list all customer credit + payments        |
| POST   | /api/debts           | add a new credit entry                     |
| POST   | /api/debts/:id/payments | record a payment against a debt         |
| DELETE | /api/debts/:id       | delete a credit entry                      |
| GET    | /api/debts/day-summary/:date | new credit given / repayments received on one date |
