...
// Computes hours worked from "HH:MM" time-in/time-out strings. Handles
// overnight shifts (e.g. 18:00 -> 02:00) by assuming time-out is the next
// day whenever it's earlier than time-in.
function computeShiftHours(timeIn, timeOut){
...
// Hours for a shift: derived from time in/out when both are set, otherwise
// falls back to a legacy manually-entered hours value (older records).
function getShiftHours(shift){
  if(shift.timeIn && shift.timeOut) return computeShiftHours(shift.timeIn, shift.timeOut);
...
let currentDay = null;
let currentDebtDay = { newCredit: 0, repayments: 0 }; // this date's new credit given / repayments received
...
async function loadDay(date){
  ...
  try{
    currentDebtDay = await apiGet('/debts/day-summary/'+date);
  }catch(e){
    currentDebtDay = { newCredit: 0, repayments: 0 };
  }
}
...
function computeDayTotals(day, debtDay){
  debtDay = debtDay || currentDebtDay || {newCredit:0, repayments:0};
...
function renderDayTab(){
  ...
  const expenseRows = (currentDay.expenses||[]).map((ex, idx)=>`
    <div class="staff-row" data-idx="${idx}">
      <input type="text" class="expense-desc" placeholder="What was it for?" value="${escapeHtml(ex.description||'')}" style="flex:2;">
      <select class="expense-cat" style="flex:1;">${EXPENSE_CATEGORIES.map(c=>`<option value="${c}" ${c===(ex.category||'Other')?'selected':''}>${c}</option>`).join('')}</select>
      <input type="number" class="expense-amount" placeholder="Amount" min="0" value="${ex.amount||''}" onfocus="this.select()" style="flex:1;">
      <button class="btn danger small remove-expense">Remove</button>
    </div>
  `).join('') || '<div class="empty-note">No expenses logged for this day yet.</div>';
  ...
  el.innerHTML = `
    ...
    <div class="card">
      <h2><span class="eyebrow">Outgoings</span> Expenses</h2>
      <div id="expenseRows">${expenseRows}</div>
      <div class="row-actions">
        <button class="btn ghost small" id="addExpenseBtn">+ Add expense</button>
        <span class="save-status" style="margin-left:auto;">Total expenses: <span id="totalExpensesVal2">${fmt(totals.totalExpenses)}</span></span>
      </div>
      ${totals.newCredit || totals.repayments ? `<p class="save-status" style="margin-top:10px;">This day also has ${totals.newCredit?`<b>${fmt(totals.newCredit)}</b> in new customer credit given`:''}${totals.newCredit && totals.repayments?' and ':''}${totals.repayments?`<b>${fmt(totals.repayments)}</b> in debt repayments received`:''} — see the Debts tab.</p>` : ''}
    </div>
  ...
  document.querySelectorAll('.remove-expense').forEach(btn=>{
    btn.onclick = (e)=>{
      const idx = Number(e.target.closest('.staff-row').dataset.idx);
      currentDay.expenses.splice(idx,1);
      renderDayTab();
    };
  });
  document.querySelectorAll('.expense-desc').forEach((inp,i)=> inp.oninput = ()=>{ currentDay.expenses[i].description = inp.value; });
  document.querySelectorAll('.expense-cat').forEach((sel,i)=> sel.onchange = ()=>{ currentDay.expenses[i].category = sel.value; });
  document.querySelectorAll('.expense-amount').forEach((inp,i)=> inp.oninput = ()=>{ currentDay.expenses[i].amount = inp.value; updateDayComputedUI(); });
...
function updateDayComputedUI(){
  ...
  (currentDay.shifts||[]).forEach((p, idx)=>{
    const hEl = document.getElementById('staffHours-'+idx);
    if(hEl) hEl.textContent = fmt(getShiftHours(p)) + ' hrs';
  });
  ...
  setText('newCreditVal', fmt(totals.newCredit));
  setText('repaymentsVal', fmt(totals.repayments));
  setText('expectedInflowVal', fmt(totals.expectedInflow));
...
// ---------- Rendering: Debts (customer credit) ----------
async function renderDebtsTab(){
  const el = document.getElementById('debtsTab');
  el.innerHTML = '<div class="loading">Loading debts…</div>';
  const debts = await apiGet('/debts');

  const totalOutstanding = debts.reduce((s,d)=> s + Math.max(0, d.outstanding), 0);
  const totalEverLent = debts.reduce((s,d)=> s + d.originalAmount, 0);
  const totalEverRepaid = debts.reduce((s,d)=> s + d.paidAmount, 0);

  const rows = debts.map(d=>{
    const isPaid = d.outstanding <= 0.005;
    return `
    <tr data-id="${d.id}">
      <td class="name-cell">${escapeHtml(d.customerName)}${d.notes?`<br><span class="save-status">${escapeHtml(d.notes)}</span>`:''}</td>
      <td>${d.dateIncurred}</td>
      <td class="computed">${fmt(d.originalAmount)}</td>
      <td class="computed">${fmt(d.paidAmount)}</td>
      <td class="computed ${d.outstanding>0?'neg':''}">${fmt(d.outstanding)}</td>
      <td><span class="status-pill ${isPaid?'ok':'bad'}">${isPaid?'Paid':'Outstanding'}</span></td>
      <td style="white-space:nowrap;">
        ${!isPaid ? `<button class="btn ghost small pay-debt-btn">Add payment</button>` : ''}
        <button class="btn danger small delete-debt-btn">Delete</button>
      </td>
    </tr>
  ...
  `;
  }).join('') || '<tr><td colspan="7" class="empty-note">No customer credit recorded yet.</td></tr>';

  el.innerHTML = `
    <div class="card">
      <h2><span class="eyebrow">Overview</span> Customer credit</h2>
      <div class="summary-grid">
        <div class="summary-item"><div class="label">Currently outstanding</div><div class="value ${totalOutstanding>0?'neg':''}">${fmt(totalOutstanding)}</div></div>
        <div class="summary-item"><div class="label">Ever lent on credit</div><div class="value">${fmt(totalEverLent)}</div></div>
        <div class="summary-item"><div class="label">Ever repaid</div><div class="value pos">${fmt(totalEverRepaid)}</div></div>
      </div>
    </div>

    <div class="card">
      <h2><span class="eyebrow">Add</span> New customer credit</h2>
      <div class="grid-2">
        <div class="field"><label>Customer name</label><input type="text" id="newDebtName" placeholder="e.g. John K."></div>
        <div class="field"><label>Date incurred</label><input type="date" id="newDebtDate" value="${currentDate}"></div>
        <div class="field"><label>Amount owed</label><input type="number" id="newDebtAmount" placeholder="Amount" min="0"></div>
        <div class="field"><label>Notes (optional)</label><input type="text" id="newDebtNotes" placeholder="e.g. 2 beers, will pay Friday"></div>
      </div>
      <div class="row-actions">
        <button class="btn brass" id="addDebtBtn">Add credit entry</button>
        <span class="save-status" id="debtSaveStatus"></span>
      </div>
    </div>
  ...
  document.getElementById('addDebtBtn').onclick = async ()=>{
    const customerName = document.getElementById('newDebtName').value.trim();
    const dateIncurred = document.getElementById('newDebtDate').value || todayStr();
    const originalAmount = document.getElementById('newDebtAmount').value;
    const notes = document.getElementById('newDebtNotes').value.trim();
    const statusEl = document.getElementById('debtSaveStatus');
    if(!customerName || originalAmount===''){
      statusEl.textContent = 'Please fill in customer name and amount.';
      return;
    }
    statusEl.textContent = 'Saving…';
    try{
      await apiPost('/debts', {customerName, dateIncurred, originalAmount, notes});
      renderDebtsTab();
    }catch(e){
  ...
  document.querySelectorAll('.pay-debt-btn').forEach(btn=>{
    btn.onclick = (e)=>{
      const id = e.target.closest('tr').dataset.id;
      const payRow = document.querySelector(`.pay-row[data-payrow="${id}"]`);
      if(payRow) payRow.style.display = payRow.style.display==='none' ? '' : 'none';
    };
  });

  document.querySelectorAll('.submit-payment-btn').forEach(btn=>{
    btn.onclick = async (e)=>{
      const payRow = e.target.closest('.pay-row');
      const id = payRow.dataset.payrow;
      const amount = payRow.querySelector('.pay-amount').value;
      const datePaid = payRow.querySelector('.pay-date').value || todayStr();
      const statusEl = payRow.querySelector('.pay-status');
      ...
      statusEl.textContent = 'Saving…';
      try{
        await apiPost(`/debts/${id}/payments`, {datePaid, amount});
        renderDebtsTab();
      }catch(err){
  ...
  document.querySelectorAll('.delete-debt-btn').forEach(btn=>{
    btn.onclick = async (e)=>{
      const id = e.target.closest('tr').dataset.id;
      if(!confirm('Delete this credit entry and its payment history? This cannot be undone.')) return;
  ...
  el.innerHTML = `
    <div class="card">
      ...
      <div class="summary-grid">
        ...
        <div class="summary-item"><div class="label">All-time expenses</div><div class="value">${fmt(data.totalExpenses)}</div></div>
...
function switchTab(name){
  ...
  ['day','inventory','debts','history','balance'].forEach(t=>{
